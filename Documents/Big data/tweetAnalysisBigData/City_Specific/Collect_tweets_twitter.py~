...
while len(searched_tweets) < max_tweets:
 ...
 #   try:
    new_tweets = api.GetSearch(term=query, count=count, max_id=last_id)
    if not new_tweets:
       break
...
f = open(query + "_tweets_id.txt", "wb")
for i, t in enumerate(tweet1s):
    print >>f, """%d""" % (
            t.id)
f.close()

f = open(query + "new_tweetsToFollowfor.html", "wb")
